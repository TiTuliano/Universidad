...
ContratoMovil::ContratoMovil(long int dni, Fecha f, float p, int m, char *nac):Contrato()
{
    this->precioMinuto=p;
...
ContratoMovil::ContratoMovil(const ContratoMovil& c):Contrato()
{
...
void ContratoMovil::ver() const
{
    ...
    cout << " " << this->minutosHablados << "m, " << this->nacionalidad << " " << this->precioMinuto<<" - "<<precioMinuto*minutosHablados<<"€";
}
